...
import com.haoxuer.adminstore.data.so.MemberSo;
...
import com.haoxuer.discover.user.data.so.UserBindSo;
...
public class MemberAction {
    ...
    public String accounts(Pageable pageable, UserBindSo so, ModelMap model) {
        ...
        pageable.getFilters().addAll(FilterUtils.getFilters(so));
        Page<UserBind> pagination = bindService.page(pageable);
        model.addAttribute("list", pagination.getContent());
        model.addAttribute("page", pagination);
        model.addAttribute("so", so);
        return "/admin/member/accounts";
    }
    ...
    private void iniRoles(ModelMap model) {
    ...
    public String save(UserRegisterRequest bean,String name, Long[] roles, ModelMap model, RedirectAttributes attributes) {

        String view = "redirect:view_list.htm";
        try {
            bean.setBindType(BindType.account);
            UserBasicResponse response = userInfoService.register(bean);
            if (response.getCode() != 0) {
                iniRoles(model);
                model.addAttribute("erro", response.getMsg());
                view = "/admin/member/add";
                return view;
            }
            if (roles != null && roles.length > 0) {
                UserInfo user = userInfoService.addRoles(response.getId(), roles);
                user.setName(name);
